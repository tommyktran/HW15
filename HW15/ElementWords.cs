...
public class ElementWords
{
    ...
    private PeriodicTable _periodicTable = new PeriodicTable();
    private static List<string> _symbols = new List<string> { };
    public ElementWords(string word)
    {
        _word = word;

        foreach (var element in _periodicTable.Elements)
        {
            _symbols.Add(element.Symbol.ToLower());
        }
    }
    public void Solve()
    {
    ...
    private static bool Spelling(List<string> result)
    {
        // If the word has been entirely worked through, output.
        if (_word == "")
        {
        ...
        // Choice 1: If the first letter of the word is a symbol in the periodic table
        if (_symbols.Contains(_word.Substring(0, 1).ToLower()))
        {
            string temp = _word.Substring(0, 1).ToUpper();
            result.Add(temp);
            string temp2 = _word;
            _word = _word.Substring(1);
            if (Spelling(result)) return true;
            _word = temp2;
            result.RemoveAt(result.Count - 1);
        }

        // Choice 2: If the first two letters of the word are a symbol in the periodic table
        if (_word.Length >= 2)
        {
            if (_symbols.Contains(_word.Substring(0, 2).ToLower()))
            {
                string temp = _word.Substring(0, 2).ToLower();
